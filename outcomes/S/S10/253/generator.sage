class Generator(BaseGenerator):
    def data(self):
        ...
        p0=p0dec*.50
        pvalue=0
        while(pvalue<0.08 or pvalue>0.12):
            if htt==0:
                x=randint(20,s-20)
            if htt==1:
            ...
            if pvalue<.1:
                conclusion="reject"
            context="The claim that more than 50 percent of adults would support adding AI literacy to high school's curricula is NOT supported."
            if conclusion=="reject":
                context="The claim that more than 50 percent of adults would support adding AI literacy to high school's curricula is supported."
        return {
        "p0dec":p0dec,
        "n":n,
        "s":s,
